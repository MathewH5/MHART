...
const progressPercentage = document.getElementById('scroll-progress-percentage');

window.addEventListener('scroll', () => {
    const windowHeight = window.innerHeight;
    const documentHeight = document.documentElement.scrollHeight;
    const scrollTop = window.scrollY;
    const scrollPercentage = ((scrollTop / (documentHeight - windowHeight)) * 100).toFixed(1); // Arredonde a uma casa decimal

    progressBar.style.width = scrollPercentage + '%';
    progressPercentage.innerText = scrollPercentage + '%';
});
